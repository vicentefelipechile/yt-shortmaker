...
- **ai (`ai/provider.rs:42`, `ai/gemini.rs:53`)**: `trait AiProvider { id, display_name, capabilities{supports_native_video}, analyze_chunk, test_connection }`. `ProviderRegistry HashMap<String, Arc<dyn AiProvider>>`. `GeminiProvider { keys, disabled, cursor }` resolves `AiConfig::resolved_model()/resolved_base_provider()` (`config/mod.rs:236`) → `generativelanguage.googleapis.com/v1beta`. Per-chunk sticky key: `analyze_with_key:342` reuses same key for `upload_file:118` + `wait_for_file_active:160` + `generate_moments:241`; on `429/RESOURCE_EXHAUSTED` (`is_quota_error:496`) marks key disabled and retries same chunk with next key via `next_key_index:98` (round-robin skipping disabled). Never mixes keys for same `file_uri`. `FILE_ACTIVE_TIMEOUT=180s` (`gemini.rs:38`) + URL normalization (`files/` single prefix) + status-aware poll (`404/400/403` bail, `429/5xx` retry) + `fileData` (not `inlineData`) + `delete_file` cleanup after wait/generate. Poll spam removed: `PROCESSING` no longer `send_log` every 2s (only `tracing::debug` cada 10 polls).
