...
**Core (lib.rs:5):** `config | media | ai | plano | session | security | setup | types`. Pattern: `core` exposes pure functions; `app` holds `Rc<RefCell<AppConfig>>` (`crates/app/src/main.rs:366`) and Slint in/out properties; long work runs in `std::thread::spawn + tokio::Runtime::new + CancellationToken` with progress via `invoke_from_event_loop`.

- **config (`crates/core/src/config/mod.rs:117`)**: `AppConfig { version:2, language, output_dir, cookies, processing{chunk_size_secs,max_last_chunk,delay,retry,auto_extract}, export{naming_template,ffmpeg_path}, ai{active_provider, providers{keys,model,model_pro,use_fast_model,temperature,media_resolution}, custom_models[], active_model_id} }`. Path `dirs::config_dir()/yt-shortmaker-v2/settings.json` (`store.rs`), atomic tmp+rename write, `normalize()` validates ranges (`max_last_chunk` now `60..=3600`, not `> chunk_size`) and migrates `model/model_pro` → `custom_models`. Default `chunk_size=600, max_last=900` no longer triggers spurious normalize.
...
- **media**: `ffmpeg.rs` `get_duration/get_resolution` + `setup::dependency_status`; `chunk.rs:21` `calculate_chunks()` + `split_video` (`-ss/-t` with cancellation); `ytdlp.rs:49` `fetch_info/download_video` (cookies, retry, NotFound→per-OS guidance) using `setup::yt_dlp_bin()`; `pipeline.rs` state machine `Stage::Downloading|Splitting|Analyzing{current,total}|Extracting` + `Pipeline::run` with resume: persistent `work_dir = session::video_work_dir(video_id)` (`session/mod.rs:142`), `PipelineEvent::Progress/Log/StageChanged`, `check_dependencies` fail-fast, and strict verification (folder+DB).
...
**Slint App (`app/ui/app.slint:198` / `app/src/main.rs:354`)**: `MainWindow` router (`route` string), route owner, `preferred 980x640 min 880x560`, sidebar `210px #18181b`, bg `#121214`, panels `#1a1a1e`, fields `#202124/#3d4149`, accent `#6366f1/#0878c9`. Sidebar has single video entry: `Inicio` (covers legacy `home`+`new-project`, `app.slint:378` `route=="home"||"new-project"`), `export|settings|keys`. No recent projects list. `Inicio` contains URL input + dedicated video panel (`video-id/duration/thumbnail/thumbnail-image/has-info`, `160x90` preview) + progress + collapsible technical log (`yt-dlp/ffmpeg`). Semantic callbacks `save-settings/save-ai-settings/browse-output/fetch-info/start-analyze/...` map to Rust with no logic in `.slint`. Translations `apply_translations()` (`locales/*.yml` `rust-i18n 4.2` → `in property <string> tr-*`). `tracing_subscriber::fmt().with_ansi(false)` (`main.rs:358`, `cli/src/main.rs:77`) and `tracing::debug!` for `config normalized` (`main.rs:377`) avoid raw ANSI `←[2m` in debug console.

**CLI (`crates/cli/src/main.rs`)**: `clap 4.6` `preview|transform|batch|analyze` (shares `core`); `analyze` now uses `session::video_work_dir(video_id)` persistent dir, not `temp/cli-*`.
